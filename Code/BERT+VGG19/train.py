...
def main():
    ...
    plt.xlabel('Epoci')
    plt.ylabel('Loss')
    plt.title('Evoluția loss-ului la antrenare')
    plt.legend()
    plt.show()
    plt.savefig('train_loss.png') 

    plt.figure(figsize=(10, 5))
    plt.plot(range(1, config.num_epochs+1), train_accs, label='Training Accuracy')
    plt.xlabel('Epoci')
    plt.ylabel('Acuratețe')
    plt.title('Evoluția acurateții la antrenare')
    plt.legend()
    plt.show()
