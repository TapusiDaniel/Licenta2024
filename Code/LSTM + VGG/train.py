...
def main():
    ...
    if config.model_type == 'early_fusion':
        ...
        model = LateFusionModel(config.num_classes, LSTMConfig(len(vocab), config.embedding_size, config.hidden_size, config.num_layers, config.dropout_prob, config.bidirectional), config.dropout_prob)
    else:
