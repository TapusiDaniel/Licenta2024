...
def main():
    ...
    if config.model_type == 'early_fusion':
        model = EarlyFusionModel(config.num_classes, config.distilbert_config, config.dropout_prob)
    elif config.model_type == 'late_fusion':
        model = LateFusionModel(config.num_classes, config.distilbert_config, config.dropout_prob)
    else:
